...
fun main() {
    ...
    // Now iterate through this array. Ignore the first. Split the rest by semi colon
    for (thing in b) {
        var red = 0;
        var blue = 0;
        var green = 0;
        ...
        val c = thing.split(":")[1]
        val id = thing.split(":")[0].split(" ")[1].toInt()
        var mini_power = 0
        var ok = true
        val event = c.split(";")
        // val balls = event.split(",")
        // println("$c")
        // println("$event")
        
        for (anotherthing in event) {
            //println("$anotherthing")

            val individual_colour = anotherthing.split(",")
            for (one_thing in individual_colour) {
                //println("$one_thing")
                val one_thing_split = one_thing.split(" ")
                val number = one_thing_split[1].toInt()
                val colour = one_thing_split[2]
